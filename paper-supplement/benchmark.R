...
for (cl in calls) {
  ...
  if (!is.null(res$UDs)) {            # Steps(): report the first stage
    runs <- res$UDs[[1]]$n
    fac  <- length(res$UDs)
  } else if (!is.null(res$Levels)) {  # Qn()
    runs <- res$V
    fac  <- res$R
  } else {                            # BIB()
    runs <- res$V
  ...
  out <- rbind(out, data.frame(
    call = cl$label, runs = runs, factors = fac, reps = n,
    seconds = round(el, 3),
    object_MB = round(as.numeric(utils::object.size(res)) / 1024^2, 2)))
}
